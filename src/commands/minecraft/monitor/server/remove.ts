...
execute(group, async (interaction) => {
    ...
    try {
        ...
        const serverStatus = await ServerStatus.findOne({ channelId: interaction.channelId });

        if (!serverStatus) {
            await interaction.editReply('❌ No Minecraft server is being monitored in this channel.');
            return;
        }
        ...
        if (serverStatus.messageId) {
            try {
                const message = await interaction.channel?.messages.fetch(serverStatus.messageId);
                if (message) {
        ...
        await interaction.editReply('✅ Minecraft server monitoring has been removed from this channel.');
    } catch (error) {
        console.error('Error removing Minecraft server monitoring:', error);
        await interaction.editReply('❌ Failed to remove Minecraft server monitoring. Please try again later.');
    }
});
